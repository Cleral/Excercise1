...
public class Match {
	...
	public Match(Connection conn, int tra_fileid, int feacount,
			double bufferdist, double adjacentdist, double limitspeed,
			double limittime) {
	...
	public boolean MapMatch(Network wh_roads) {
		CallableStatement cs_percentage = null;
		CallableStatement cs_distance = null;
		try {
			cs_percentage = conn.prepareCall("{ call getpercentage(?,?,?,?) }");
			cs_distance = conn.prepareCall("{ call getdistance(?,?,?) }");

			// Network wh_roads = NetworkManager.readNetwork(conn, "wh_roads");
			// //加载网络
			...
			while (rs_tra.next()) {
				...
				while (rs_links.next()) {
					...
					Struct struct_link = (Struct) rs_links
							.getObject("street_geom");
					...
					double startper = GetPercentage("wh_roads2", link_id,
							cpt.struct, cs_percentage);

					// 观测概率
					double mp_cost = 1.0 / (Math.sqrt(2 * Math.PI) * 9)
							* Math.exp(-0.5 * Math.pow((cpt.dist / 9), 2))
							* constvalue;
					Tra_Node pNode = new Tra_Node(-1, tra_id, link_id, fileid,
							time, mp_cost, struct_point, geom_point,
							cpt.struct, cpt.geo, struct_link, geo_link,
							start_node_id, end_node_id, oneway, link_length,
							startper, null, null, -1);
				...
				// }
				// String sql_link =
				// " select l1.link_id,l1.street_geom,l1.start_node_id,l1.end_node_id,"
				// +
				// "l1.street_length,l1.BIDIRECTED from links l1 where l1.link_id = ? or "
				// + "l1.start_node_id = ? or l1.end_node_id = ?";
				// ps =
				// conn.prepareStatement(sql_link,ResultSet.TYPE_SCROLL_INSENSITIVE,
				// ResultSet.CONCUR_UPDATABLE);
				...
				if (flag == 0) {
					invalidPoint.add(new Tra_Node(-1, tra_id, -1, fileid, time,
							-1, struct_point, geom_point, null, null, null,
							null, -1, -1, null, -1, -1, null, null, -1));
				...
				if (_index != 0) {
					...
					long timespan = Math.abs(date.getTime()
							- date_before.getTime()) / 1000;
					...
					// double dist_Adjacent =
					// geom_point.distance(pTree.get(_index-1).get(0).geo_old,
					// 0.05, "true");

					// CallableStatement cs = null;
					double dist_Adjacent = getDistance(struct_point,
							pTree.get(_index - 1).get(0).st_old, conn,
							cs_distance);
					// ************************************
					if (dist_Adjacent == 0) {
					...
					if (dist_Adjacent < adjacentdist) {
						adjacentPoint.add(new AdjacnetPoint(pTree.get(
								_index - 1).get(0).id, struct_point,
								geom_point, tra_id, fileid, time)); // 此处只保存上一结点的ID，这样就可以通过modifiedGPS数组将结果移植过来
						continue; // 小于设置的相邻距离 *****此处需要处理，将此点结果设置为上一点的
					}
					// 将时间异常的点提取出来 **************** timespan < 5 是否正确？
					if ((dist_Adjacent / timespan) > (limitspeed / 3.6)
							&& timespan < 10) {
						timeExceptionPoint.add(new Tra_Node(-1, tra_id, -1,
								fileid, time, -1, struct_point, geom_point,
					...
					for (int j = 0; j < gpsNodes.size(); j++) {
						...
						for (int i = 0; i < pTree.get(_index - 1).size(); i++) {
							...
							if (NetworkManager.isReachable(wh_roads,
									before.link_startNodeID,
									gpsNodes.get(j).link_endNodeID)) {
								if (before.link_id == gpsNodes.get(j).link_id) {
									if (before.oneway.equals("N")) {
										if (gpsNodes.get(j).percentage
												- before.percentage < 0) {
											dist_route = 0;
										} else {
											dist_route = (gpsNodes.get(j).percentage - before.percentage)
													* before.link_length;
										}
									} else {
										dist_route = Math
												.abs(gpsNodes.get(j).percentage
														- before.percentage)
												* before.link_length;
									}
								} else {
									// ****************************************************
									if ((before.percentage > 0 && before.percentage < 1)
											|| (gpsNodes.get(j).percentage > 0 && gpsNodes
													.get(j).percentage < 1)) {
										subpath = GetSubPath(wh_roads, before,
												gpsNodes.get(j), timespan
														* (limitspeed / 3.6));
									} else {
										int startNodeID = -1;
										...
										if (startNodeID != -1
											...
											if (startNodeID == endNodeID) {
												...
												Path path = NetworkManager
														.shortestPath(wh_roads,
																startNodeID,
																endNodeID);
												if (path != null) {
							...
							// 路径距离为0 或者超速 下一个
							if (dist_route == 0
									|| (dist_route / timespan) > (limitspeed / 3.6)) {
								continue;
							}
							invalidflag = true; // 此点有路可达 有效
							double dt = Math.abs(dist_Adjacent - dist_route);
							double tp_cost = 0.039 * Math.exp(-dt * 0.039)
									* constvalue; // 转移概率
							if (cost == 0) {
								cost = gpsNodes.get(j).cost * tp_cost
										* before.cost;
								// gpsNodes.get(j).cost = cost;
								gpsNodes.get(j).root_index = i;
								if (subpath != null) {
									gpsNodes.get(j).path = subpath
											.getReferencePath();
								} else {
								...
								double temp = gpsNodes.get(j).cost * tp_cost
										* before.cost;
								if (temp > cost) {
									cost = temp;
									// gpsNodes.get(j).cost = cost;
	 ...
	 * @return 最短路径
	 */
	private SubPath GetSubPath(Network wh_roads, Tra_Node before,
			Tra_Node current, double MaxCost) {
		SubPath subpath = null;
		try {
			double startPercentage = before.percentage;
			double endPercentage = current.percentage;
			subpath = NetworkManager.shortestPath(wh_roads, before.link_id,
					startPercentage, current.link_id, endPercentage, null); // new
																			// CostConstraint(MaxCost)
			return subpath;
	 ...
	 * @param invalidPoint
	 *            缓冲区内没有道路的点
	 * @param timeExceptionPoint
	 *            时间异常的点
	 * @param adjacentPoint
	 *            相邻两点距离太短的点
	 */
	private void WriteRoute(List<List<Tra_Node>> pTree,
			List<Tra_Node> invalidPoint, List<AdjacnetPoint> adjacentPoint)
			throws SQLException {
		if (pTree.size() == 0) {
			return;
		}
		...
		try {
			...
			for (int j = pTree.size() - 1; j >= 0; j--) {
				...
				tra[pTree.get(j).get(pNode_Index).id] = pTree.get(j).get(
						pNode_Index);
			...
			for (int i = 0; i < adjacentPoint.size(); i++) {
				...
				ClosestPoint cpt = GetCPTAndDist(adjacentPoint.get(i).st_point,
						st_link); // 获取最近点和距离
				...
				ps_old.addBatch();
				tra[adjacentPoint.get(i).tra_id] = new Tra_Node(-1,
						adjacentPoint.get(i).tra_id,
						tra[beforepointid].link_id,
						adjacentPoint.get(i).fileid, adjacentPoint.get(i).time,
						-1, adjacentPoint.get(i).st_point,
						adjacentPoint.get(i).geo_point, cpt.struct, cpt.geo,
						st_link, tra[beforepointid].geo_link,
						tra[beforepointid].link_startNodeID,
						tra[beforepointid].link_endNodeID,
						tra[beforepointid].oneway,
	 ...
	 *            时间异常点
	 */
	private void SolveTimeExceptionPoint(
			Hashtable<Integer, List<Tra_Node>> allNodes,
			List<Tra_Node> timeExceptionPoint, Network wh_roads) {
		String sql_new = "insert into wh_modifiedTra(new,tra_id,file_id,time,roadid) values(?,?,?,?,?)";
		String sql_old = "insert into wh_tras_old(origin,tra_id,file_id,time,road_id) values(?,?,?,?,?)";

		// 防止游标溢出
		CallableStatement cs_closestLink = null;
		CallableStatement cs_distance = null;
		...
		try {
			...
			cs_closestLink = conn
					.prepareCall("{ call getclosestlinkid2(?,?) }");
			cs_distance = conn.prepareCall("{ call getdistance(?,?,?) }");
					...
					if (tra[i].beforeid != -1 && tra[i].beforeid != i - 1) {
						...
						for (int j = beforeid + 1; j < i; j++) {
							...
							if (st_old != null && index != -1) {
								...
								ps_old.addBatch();
								ps_old.executeBatch();
								tra[j] = new Tra_Node(-1, j, linkid, fileid,
										time, -1, st_old, geo_old, cpt.struct,
										cpt.geo, null, null, -1, -1, null, -1,
										-1, null, null, -1);
							}
						}
						i = tra[i].beforeid + 1;
					} else if (i != 0 && tra[i].beforeid == -1
							&& tra[i - 1] == null) {
						int CurrentTraid = i;
						int NextTraid = -1;
						...
						if (NextTraid != -1) {
							...
							if (NetworkManager.isReachable(wh_roads,
									before.link_startNodeID,
									tra[CurrentTraid].link_endNodeID)) {
								if (before.link_id == tra[CurrentTraid].link_id) {
									st = before.st_link;
								} else {
									// ****************************************************
									if ((before.percentage > 0 && before.percentage < 1)
											|| (tra[CurrentTraid].percentage > 0 && tra[CurrentTraid].percentage < 1)) {
										SubPath subpath = GetSubPath(wh_roads,
												before, tra[CurrentTraid], 5000);
										if (subpath != null) {
											subpath.computeGeometry(0.05);
											JGeometry geo = subpath
													.getGeometry();
							...
							for (int p = NextTraid + 1; p < CurrentTraid; p++) {
								...
								ClosestPoint cpt = GetCPTAndDist(tra[p].st_old,
										st);
			...
			for (int n = 0; n < remain.size(); n++) {
				...
				if (before != null) {
					for (int j = 0; j < gpsNodes.size(); j++) {
						...
						double dist_route = 0;
						// double dist_Adjacent =
						// before.geo_old.distance(gpsNodes.get(j).geo_old,
						// 0.05, "true");

						// CallableStatement cs = null;
						double dist_Adjacent = getDistance(before.st_old,
								gpsNodes.get(j).st_old, conn, cs_distance);
						if (NetworkManager.isReachable(wh_roads,
								before.link_startNodeID,
								gpsNodes.get(j).link_endNodeID)) {
							if (before.link_id == gpsNodes.get(j).link_id) {
								...
								if ((before.percentage > 0 && before.percentage < 1)
										|| (gpsNodes.get(j).percentage > 0 && gpsNodes
												.get(j).percentage < 1)) {
									subpath = GetSubPath(wh_roads, before,
											gpsNodes.get(j), dist_Adjacent * 5);
								} else {
									int startNodeID = -1;
									if (before.percentage == 0) {
										startNodeID = before.link_startNodeID;
			...
			if (cs_closestLink != null)
				try {
					cs_closestLink.close();
				} catch (Exception e) {
				}
			if (cs_distance != null)
				try {
	...
	/**
	 * 2017-2-14修改，防止由于cs的每一次实例化都会打开一个游标，导致游标溢出
	 * 
	 * @param tra
	 *            原始轨迹点
	 * @param conn
	...
	private int getCloestlinkid(Struct tra, Connection conn,
		...
		try {
			// cs = conn.prepareCall( "{ call getclosestlinkid2(?,?) }" );
			cs.setObject(1, tra);
	...
	private class AdjacnetPoint {
		 ...
		 *            本点采集时间
		 */
		public AdjacnetPoint(int _before_point_id, Struct _st_point,
				JGeometry _geo_point, int _tra_id, int _fileid, String _time) {
	...
	private double GetPercentage(String networkname, int link_id, Struct pt,
		...
		double percentage = 0;
		try {
			// cs = conn.prepareCall("{ call getpercentage(?,?,?,?) }");
	...
	private ClosestPoint GetCPTAndDist(Struct struct_point, Struct struct_line) {
		ClosestPointAndDist CPTAndDist = new ClosestPointAndDist(conn,
				struct_point, struct_line);
	...
	private double getDistance(Struct p1, Struct p2, Connection conn,
		...
		double dis = 0;
		try {
			// cs = conn.prepareCall("{ call getdistance(?,?,?) }");
			cs.setObject(1, p1);
			cs.setObject(2, p2);
